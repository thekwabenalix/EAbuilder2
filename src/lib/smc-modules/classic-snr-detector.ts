 ...
 * ────────────────────────────────────────────────
 * Classic Support / Resistance levels from candle-pair direction reversal.
 *
 * SNR PRICE = Candle A close.
 *
 * CLASSIC RESISTANCE:  Bullish Candle A → Bearish Candle B
 * CLASSIC SUPPORT:     Bearish Candle A → Bullish Candle B
 *
 * LIFECYCLE STATES:
 *   ACTIVE  → no touch yet
 ...
 *   OBJ_TEXT label "C-Sup" / "C-Res" at the left anchor.
 *
 * JOURNAL:
...
//|   SUPPORT:    Bearish candle A → Bullish candle B              |
...
input bool            InpShowResistance = true;           // Show resistance levels
...
{ return t == TYPE_SUPPORT ? "CLASSIC_SUPPORT" : "CLASSIC_RESISTANCE"; }
...
string ObjLine(int id) { return "SMCCSNR_" + IntegerToString(id) + "_ln"; }
string ObjLbl (int id) { return "SMCCSNR_" + IntegerToString(id) + "_lb"; }
...
//| Creates a Classic SNR level when the two candles reverse.       |
//+------------------------------------------------------------------+
void CheckSnrPair(int shA, int shB)
{
   ...
   if(dirA < 0 && dirB > 0) snrType = TYPE_SUPPORT;    // Bear A → Bull B
   if(snrType == 0)          return;                    // same direction = Gap SNR, not Classic
...
void UpdateLifecycleAtBar(int sh)
{
   ...
   for(int i = 0; i < snrTotal; i++)
   {
      ...
      if(isSupport)
      {
         if(barClose < lvl) // close below support = broken
         {
      ...
      {
         if(barClose > lvl) // close above resistance = broken
         {
            snrList[i].state   = STATE_BROKEN;
            snrList[i].endTime = barT;
            if(InpShowLog)
...
void SNR_DrawOne(int idx)
{
   ...
   // Delete objects first (clean update)
   ...
   // Line endpoints: active extends right; broken/expired stops at endTime
   ...
   bool ray = active; // RAY_RIGHT=true → line extends to the right automatically
...
void DeleteAllObjects()
{
   for(int i = ObjectsTotal(0) - 1; i >= 0; i--)
   {
      ...
      if(StringFind(nm, "SMCCSNR_") == 0) ObjectDelete(0, nm);
   }
}
...
int OnInit()
{
   ...
   int avail = iBars(_Symbol, InpTF);
   if(avail < 3) { Print("Classic_SNR_Detector: not enough bars."); return INIT_FAILED; }
   ...
   // Trim to max visible after full history is evaluated
   EnforceMaxLevels();
   SNR_DrawAll();
   ...
   PrintFormat("Classic_SNR_Detector v1 ready | active=%d touched=%d broken=%d expired=%d | %s %s",
                ...
                const long &volume[], const int &spread[])
{
   ...
   // Update lifecycle for the just-closed bar
   UpdateLifecycleAtBar(1);

   // Enforce visual cap, redraw
   EnforceMaxLevels();
   SNR_DrawAll();

...
function test1_ClassicICT(): TestResult {
  const config: FourBrainConfig = {
    ...
    management: { riskPercent: 1, rewardRisk: 2, stopBuffer: 20, breakEvenEnabled: false, breakEvenAtR: 1, maxOpenTrades: 1 },
  };
  const params: MQL5CodeGenParams = { eaName: "ClassicICT_Test", config, globalSymbol: "EURUSD", globalMagic: 1001 };
  ...
  return runChecks("Test 1 — Classic ICT (CHoCH D1 → OB H4 → FVG M15)", code, [
    ...
    { desc: "Direction_Brain_Execute defined",    test: code.includes("void Direction_Brain_Execute()") },
    { desc: "Setup_Brain_Execute defined",        test: code.includes("void Setup_Brain_Execute()") },
    { desc: "Execution_Brain_Execute defined",    test: code.includes("void Execution_Brain_Execute()") },
    ...
    { desc: "Confluence gate checks gBias",       test: code.includes("gBias == 0") || code.includes("gBias != 0") },
    ...
    { desc: "No unresolved {{ }} placeholders",   test: !code.includes("{{") && !code.includes("}}") },
    { desc: "CHoCH detection in direction brain", test: code.includes("_swH") && code.includes("CHoCH") },
    { desc: "OB detection in setup brain",        test: code.includes("SETUP") && code.includes("OB") },
    { desc: "FVG detection in execution brain",   test: code.includes("FVG") && code.includes("gExecDir") },
...
function test2_MultiModuleDirection(): TestResult {
  ...
  const params: MQL5CodeGenParams = { eaName: "BOS_iFVG_Test", config, globalSymbol: "EURUSD", globalMagic: 1002 };
  const code = generateEA(params);
  ...
  return runChecks("Test 2 — Multi-module Direction: BOS + FVG_INVERSION AND logic", code, [
    ...
    { desc: "BOS detection present (_swH/_swL)",  test: code.includes("_swH") && code.includes("_swL") },
    ...
    { desc: "BullJustConfirmed in direction",     test: code.includes("IFVGSM_H1_BullJustConfirmed()") },
    { desc: "BearJustConfirmed in direction",     test: code.includes("IFVGSM_H1_BearJustConfirmed()") },
    { desc: "gBias set to _combined after AND",   test: code.includes("gBias = _combined") },
    { desc: "Setup disabled — passthrough active",test: code.includes("gSetupActive = (gBias != 0)") },
    { desc: "No unresolved {{ }} placeholders",   test: !code.includes("{{") && !code.includes("}}") },
  ]);
}
...
function test3_ExecutionOnly(): TestResult {
  const config: FourBrainConfig = {
    ...
    management: { riskPercent: 0.5, rewardRisk: 1.5, stopBuffer: 10, breakEvenEnabled: false, breakEvenAtR: 1, maxOpenTrades: 2 },
  };
  const params: MQL5CodeGenParams = { eaName: "ExecOnly_Test", config, globalSymbol: "GBPUSD", globalMagic: 1003 };
  const code = generateEA(params);

  return runChecks("Test 3 — Execution-only (no direction, no setup)", code, [
    { desc: "Direction Brain disabled (empty function)", test: code.includes("Direction Brain: disabled") },
    ...
    { desc: "Trade execution still present",             test: code.includes("trade.Buy(") || code.includes("trade.Sell(") },
    { desc: "Max trades input uses 2",                   test: code.includes("input int             InpMaxTrades   = 2") },
    { desc: "No unresolved {{ }} placeholders",          test: !code.includes("{{") && !code.includes("}}") },
  ]);
}
...
function test4_IfvgStateMachineScoping(): TestResult {
  ...
  return runChecks("Test 4 — iFVG SM scoping (exec-only / none / both)", code_exec, [
    { desc: "4a: IFVGSM_M5_ injected when exec=fvg_inversion",      test: code_exec.includes("IFVGSM_M5_") },
    { desc: "4a: IFVGSM_H4_ NOT injected (dir uses bos, not iFVG)", test: !code_exec.includes("IFVGSM_H4_") },
    { desc: "4b: No IFVGSM_ when iFVG not selected anywhere",       test: !code_none.includes("IFVGSM_") },
    { desc: "4c: IFVGSM_H1_ injected when dir=fvg_inversion",       test: code_both.includes("IFVGSM_H1_") },
    { desc: "4c: IFVGSM_M5_ injected when exec=fvg_inversion",      test: code_both.includes("IFVGSM_M5_") },
...
function test5_TradeExecution(): TestResult {
  ...
  const params: MQL5CodeGenParams = { eaName: "BE_Risk_Test", config, globalSymbol: "USDJPY", globalMagic: 9999 };
  const code = generateEA(params);
  ...
  return runChecks("Test 5 — Break-even + risk/RR wired correctly", code, [
    ...
    { desc: "TP = dist * InpRewardRisk * pt",     test: code.includes("InpRewardRisk") && code.includes("dist") },
    ...
    { desc: "CountPositions >= InpMaxTrades gate",test: code.includes("CountPositions() >= InpMaxTrades") },
    { desc: "CalcLot uses InpRiskPercent",        test: code.includes("InpRiskPercent / 100.0") },
    { desc: "EMA detection in direction brain",   test: code.includes("_fast") && code.includes("_slow") },
    { desc: "S/R detection in setup brain",       test: code.includes("swH") || code.includes("support") },
    { desc: "Pin Bar detection in exec brain",    test: code.includes("PIN") && code.includes("_rng") },
    { desc: "No unresolved {{ }} placeholders",   test: !code.includes("{{") && !code.includes("}}") },
  ]);
}

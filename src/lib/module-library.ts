...
export const MODULE_LIBRARY: ModuleSpec[] = [
  ...
  // ─── EMA ────────────────────────────────────────────────────────────────────
  {
    ...
    detectionLogic: "Real iMA handles (drawn on the chart via B4_MA). TWO usage modes: (1) SIMPLE CROSS for Direction — fast vs slow alignment checked inline every bar, persistent (no state machine). (2) RETEST SEQUENCE for Setup+Execution — the verified EMASM state machine persists IDLE → ARMED (price retests the slow EMA within tolerance in the bias direction; the retest bar only arms) → CONFIRMED (a LATER bar closes outside the fast EMA in the bias direction → entry next bar, SL = swing). Use EMASM for any multi-bar 'retest then close outside' rule — never hand-write it inline (the phases collapse onto one bar).",
    roles: [
      { role: "direction", fit: "primary",   usage: "Fast > slow = BULL bias. Fast < slow = BEAR bias. Inline B4_MA or EMASM_{id}_Bias()." },
      { role: "setup",     fit: "primary",   usage: "EMASM retest: RetestActive() = a pullback to the slow EMA is armed in the bias direction." },
      { role: "execution", fit: "primary",   usage: "EMASM confirmation: JustConfirmed() = a bar closed outside the fast EMA after the retest → entry." },
    ],
    lifecycle: "Simple cross: persistent alignment. Retest SM: IDLE → ARMED (retest slow EMA) → CONFIRMED (close outside fast EMA) → consumed; invalidated on bias flip or close back through the slow EMA.",
    params: [
    ...
    outputStates: [
      ...
      { name: "RetestActive()",    meaning: "Pullback to slow EMA armed", tradingImplication: "Setup live — await confirmation" },
      { name: "JustConfirmed()",   meaning: "Close outside fast EMA after retest", tradingImplication: "ENTRY in bias direction" },
    ],
    inlineApi: {
      tick: "EMASM_{id}_Tick(gBias)   // retest mode; simple cross uses inline B4_MA, no tick",
      signals: [
        { fn: "EMASM_{id}_Bias()",          returns: "int",    meaning: "Own fast/slow alignment (1/-1/0)" },
        { fn: "EMASM_{id}_RetestActive()",  returns: "bool",   meaning: "Armed retest of the slow EMA (setup)" },
        { fn: "EMASM_{id}_ActiveDir()",     returns: "int",    meaning: "Direction of the armed pullback" },
        { fn: "EMASM_{id}_ActiveSL()",      returns: "double", meaning: "Swing SL hint while armed" },

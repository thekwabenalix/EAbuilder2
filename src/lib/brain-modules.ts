...
export const ALL_BRAIN_MODULES: BrainModuleDef[] = [
  ...
  // Trend
  { id: "ema",             label: "EMA",             desc: "Fast/slow exponential moving average alignment",   symbol: "~", color: "text-cyan-400",   category: "Trend"      },
];

export const MODULE_BY_ID = Object.fromEntries(
  ALL_BRAIN_MODULES.map((m) => [m.id, m])
) as Record<BrainModuleType, BrainModuleDef>;
...
export function formatBrain(
  ...
  if (!brain) return "—";
  const mods = brain.modules
    .map((m) => MODULE_BY_ID[m]?.label ?? m.toUpperCase())
    .join(" + ");

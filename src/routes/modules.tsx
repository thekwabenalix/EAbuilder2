...
const PHASE1_MODULES: ModuleEntry[] = [
  {
    ...
    name: "FVG Detector",
    description: "Detects 3-candle Fair Value Gaps and manages full zone lifecycle — ACTIVE → MITIGATED → INVALIDATED / EXPIRED — with auto-fade and auto-removal.",
    rules: [
      "Bullish FVG: C3.Low > C1.High  →  UL = C3.Low,  LL = C1.High",
      "Bearish FVG: C3.High < C1.Low  →  UL = C1.Low,  LL = C3.High",
      "C1 = shift 3 (oldest),  C2 = shift 2,  C3 = shift 1 (newest closed)",
      "Invalidated: Bullish → close < LL  |  Bearish → close > UL",
      "Mitigated:   Bullish → high ≥ UL   |  Bearish → low ≤ LL",
    ],
    output: [
      "ACTIVE zones: full-colour filled rectangle + id label",
      "MITIGATED zones: faded rectangle (configurable per input)",
      "INVALIDATED / EXPIRED: removed or frozen + dotted border",
      "Journal: FVG_CREATED | FVG_MITIGATED | FVG_INVALIDATED | FVG_EXPIRED",
      "Inputs: remove_invalidated · fade_mitigated · expiry_bars",
    ],
    status: "ready",

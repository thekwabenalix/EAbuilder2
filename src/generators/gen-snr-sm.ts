...
void ${P}Reset()
{
   for(int _oi = ObjectsTotal(0) - 1; _oi >= 0; _oi--)
   {
      ...
      if(StringFind(_on, "4B_SNR_${tf}_") == 0) ObjectDelete(0, _on);
   }
   ${P}levelCount     = 0;
...
void ${P}Advance(int sh)
{
   ...
   // ── Chart visualization: horizontal line per level ───────────────
   datetime _t2 = iTime(InpSymbol, PERIOD_CURRENT, 0) + PeriodSeconds(${TF}) * 20;
   for(int _k = 0; _k < ${P}levelCount; _k++)
   {
      string _ln = StringFormat("4B_SNR_${tf}_%d", (int)${P}levels[_k].levelTime);
      ...
      color _col = ${P}levels[_k].state == ${P}RETESTED   ? clrGold
                 ...
                 : ${P}levels[_k].dir   == 1               ? clrCornflowerBlue
                 :                                           clrSalmon;
